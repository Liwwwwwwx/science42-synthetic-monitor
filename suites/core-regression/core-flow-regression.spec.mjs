...
async function answerFor(page, question) {
  ...
  const onResponse = response => {
    if (response.url().includes('/api/') && response.request().method() !== 'OPTIONS') {
      responses.push({ url: response.url(), status: response.status(), atMs: Date.now() - started });
    }
  };
  ...
  while (Date.now() < deadline) {
    ...
    const generating = /生成中|Generating/i.test(lastMain.slice(-500));
...
test('CORE regression: login, send, stream, result, context, save and restore', async ({ page }, testInfo) => {
  ...
  const summary = {
    environment: getTargetUrl(),
    records,
    beforeLength: before.length,
    afterLength: after.length,
    restored,
    completed: records.filter(r => r.passed).length,
  ...
  await finishSuiteReport({
    ...
    checks: [
      ...records.map((r) => ({
        ...
        errorCode: r.passed ? null : (r.timedOut ? 'TIMEOUT' : 'ASSERT_FAILED'),
