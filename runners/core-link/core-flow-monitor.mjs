...
async function sendReport(report, evidence) { if (!reportUrl || !runnerId || !runnerToken) throw new Error('Synthetic report endpoint or runner credentials are not configured'); const form = new FormData(); form.set('payload', JSON.stringify(report)); if (evidence.screenshot) form.set('screenshot', new Blob([await fs.readFile(evidence.screenshot)], { type: 'image/png' }), path.basename(evidence.screenshot)); const response = await fetch(`${reportUrl.replace(/\/$/, '')}/api/synthetic-monitoring/runners/${encodeURIComponent(runnerId)}/runs`, { method: 'POST', headers: { 'X-Synthetic-Monitor-Token': runnerToken }, body: form, signal: AbortSignal.timeout(30_000) }); if (!response.ok) throw new Error(`Synthetic report rejected: HTTP ${response.status}`); }
...
try { browser = await chromium.launch({ headless: process.env.HEADLESS !== 'false' }); context = await browser.newContext({ storageState }); page = await context.newPage(); let input; const authAt = Date.now(); try { input = await ensureChat(page); checks.push(makeCheck('auth_state', 'passed', authAt)); } catch (error) { checks.push(makeCheck('auth_state', 'failed', authAt, { errorCode: classify(error), message: brief(error) })); }
...
} catch (error) { for (const key of ['auth_state', 'chat_stream', 'session_restore']) if (!checks.some((item) => item.key === key)) checks.push(makeCheck(key, 'error', Date.now(), { errorCode: classify(error), message: brief(error) })); } finally { const screenshot = path.join(runDir, 'page.png'); await page?.screenshot({ path: screenshot, fullPage: true }).catch(() => {}); await browser?.close().catch(() => {}); }
const status = checks.every((item) => item.status === 'passed') ? 'passed' : (checks.some((item) => item.status === 'error') ? 'error' : 'failed'); const report = { runId, suiteId: 'core_link', sequence: Number(process.env.SYNTHETIC_MONITOR_SEQUENCE || Date.now()), startedAt: startedAt.toISOString(), finishedAt: new Date().toISOString(), status, durationMs: Date.now() - startedAt.getTime(), checks, errorSummary: status === 'passed' ? null : checks.filter((item) => item.status !== 'passed').map((item) => `${item.key}:${item.errorCode}`).join(', ') }; await fs.writeFile(path.join(runDir, 'result.json'), JSON.stringify(report, null, 2)); const evidence = status === 'passed' ? {} : { screenshot: path.join(runDir, 'page.png') }; let delivered = false; try { await sendReport(report, evidence); delivered = true; } catch { await spool(report, evidence); } await cleanupLocalArtifacts({ delivered, status }).catch(() => {}); console.log(JSON.stringify({ runId, status, checks: checks.map(({ key, status: checkStatus, errorCode }) => ({ key, status: checkStatus, errorCode })) })); if (status !== 'passed') process.exitCode = 1;
